...
class MobileNetV3(nn.Module):
    def __init__(self, cfgs, mode, num_classes=1000, width_mult=1.):
        ...
        # For CIFAR we need to remove some of the pooling
        layers = [conv_3x3_bn(3, input_channel, 1)]
        # building inverted residual blocks
        for k, t, c, use_se, use_hs, s in self.cfgs:
            output_channel = _make_divisible(c * width_mult, 8)
            exp_size = _make_divisible(input_channel * t, 8)
            layers.append(InvertedResidual(input_channel, exp_size, output_channel, k, s, use_se, use_hs))
...
def mobilenetv3_small(**kwargs):
    ...
    #     [5,    6,  96, 1, 1, 1],
    # ]
    # removing the first pooling (stride=2) so the final feature map size is 4x4 instead of 1x1
    cfgs = [
        # k, t, c, SE, HS, s
        [3,    1,  16, 1, 0, 1],
        [3,  4.5,  24, 0, 0, 2],
